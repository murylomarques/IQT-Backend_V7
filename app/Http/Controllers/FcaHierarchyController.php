...
class FcaHierarchyController extends Controller
{
    ...
    public function link(Request $request)
    {
        ...
        $allowed = [
            ...
            'gerente'     => ['coordenacao'],
            'admin'       => ['tecnico', 'supervisao', 'coordenacao', 'gerente'],
        ];
    ...
    public function approveLinkRequest(Request $request, $id)
    {
        ...
        if (!$linkReq->requested_at || $linkReq->requested_at->format('m/Y') !== now()->format('m/Y')) {
            return response()->json(['error' => 'Esta solicitação é de um mês anterior e expirou. Peça um novo vínculo.'], 422);
        }
